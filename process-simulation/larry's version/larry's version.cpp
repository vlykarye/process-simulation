...
#include <iomanip>
#include <vector>
#include <queue> // std::queue and std::priority queue
#include <climits>
#include <map>
...
namespace Hw1 {
string const TASKS_STRING[] = {"NCORES", "NEW", "CORE", "DISK", "DISPLAY", "INPUT"};
enum class TASKS { NCORES, NEW, CORE, DISK, DISPLAY, INPUT, TERMINATE, NONE};
string const STATE_STRING[] = {"RUNNING", "READY", "BLOCKED", "TERMINATED"};
enum class STATES { RUNNING, READY, BLOCKED, TERMINATED};

TASKS convert_to_token(string str) {
	if (str == "NCORES")
		return TASKS::NCORES;
	if (str == "NEW")
		return TASKS::NEW;
